...
class PlacesListVM: ObservableObject{
    ...
    func fetchPlaces(){
        ...
        places = CoreDataManager.shared.getAll().map({ place in
            PlaceViewModel(id: place.id!, name: place.name! , city: place.city!, country: place.country!, notes: place.notes ?? "", placeImage: Image(uiImage:UIImage(data:place.image!)!) ,flag: "🇵🇱",address: "")
        })
    }
}
